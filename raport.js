(function () {
  ...
  function rysujMapy(bbox) {
    ...
    // MAPA 0: Zaznaczenie dzialki — ciasny widok z warstwa dzialek ewidencyjnych
    const mbZazn = margines(bbox, 0.25);
    const sZazn = mbZazn.join(',');
    const zazn = wymiary(mbZazn);
    const dzialkiUrl = 'https://mapy.geoportal.gov.pl/wss/service/PZGIK/EGIB/WMS/PobranieBezplatne?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:4326&FORMAT=image/png&TRANSPARENT=true&LAYERS=dzialki,numery_dzialek&STYLES=&WIDTH=' + zazn.W + '&HEIGHT=' + zazn.H + '&BBOX=' + sZazn;
    setMapaOverlay('map-zaznaczenie', ortoBase(sZazn, zazn), dzialkiUrl);

    // MAPA 6: Linie energetyczne (BDOT10k lub OSM przez WMS)
    const energiaUrl = 'https://mapy.geoportal.gov.pl/wss/service/PZGIK/BDOT/WMS/PobranieBezplatne?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:4326&FORMAT=image/png&TRANSPARENT=true&LAYERS=siec_elektroenergetyczna&STYLES=&WIDTH=' + ortoC.W + '&HEIGHT=' + ortoC.H + '&BBOX=' + sCeny;
    setMapaOverlay('map-energia', ortoBase(sCeny, ortoC), energiaUrl);

    // MAPA 7: Ochrona przyrody (Geoserwis GDOS)
    ...
    setMapaOverlay('map-przyroda', ortoBase(sCeny, ortoC), przyrodaUrl);
    ...
    setMapaOverlay('map-zabytki', ortoBase(s, orto), zabytkiUrl);
  }
