(function () {
  ...
  function rysujObrys(idMapy, wkt, bboxMapy, wh, pokazWymiary) {
    ...
    pierscienie.forEach(function (p) {
      ...
      // WYMIARY jako LINIE WYMIAROWE (styl geodezyjny, jak OnGeo) — tylko gdy pokazWymiary.
      // Linia wymiarowa biegnie wzdluz boku, z kreseczkami na koncach i liczba przy niej.
      if (pokazWymiary && punkty.length >= 3) {
        ...
        // Najdluzsza os dzialki (para najdalszych wierzcholkow)
        ...
        const px = -ay / alen, py = ax / alen;         // kierunek prostopadly (metry)
        let minPerp = 1e9, maxPerp = -1e9, iMin = 0, iMax = 0;
        mp.forEach(function (m, idx) {
          const proj = m.mx * px + m.my * py;
          if (proj < minPerp) { minPerp = proj; iMin = idx; }
          if (proj > maxPerp) { maxPerp = proj; iMax = idx; }
        });
        ...
        // Wektor osi w PIKSELACH (do rysowania linii wymiarowej)
        const pdx = mp[ib].x - mp[ia].x, pdy = mp[ib].y - mp[ia].y;
        const plen = Math.hypot(pdx, pdy) || 1;
        const upx = pdx / plen, upy = pdy / plen;        // wzdluz osi (piksele)
        const npx = -upy, npy = upx;                     // prostopadly (piksele)

        // Odsun linie wymiarowe na zewnatrz dzialki
        const odsun = 26;  // piksele

        // LINIA DLUGOSCI — wzdluz osi glownej, odsunieta na zewnatrz
        etykiety += liniaWymiarowa(
          mp[ia].x + npx * odsun, mp[ia].y + npy * odsun,
          mp[ib].x + npx * odsun, mp[ib].y + npy * odsun,
          Math.round(dl) + ' m'
        );

        // LINIA SZEROKOSCI — miedzy skrajnymi punktami w poprzek, odsunieta w drugą stronę
        etykiety += liniaWymiarowa(
          mp[iMin].x - upx * odsun, mp[iMin].y - upy * odsun,
          mp[iMin].x - upx * odsun + npx * (szerokosc / dl * plen), mp[iMin].y - upy * odsun + npy * (szerokosc / dl * plen),
