(function () {
  ...
  function rysujMapy(bbox, wkt) {
    ...
    // MAPA: PLAN OGOLNY GMINY (POG) — uchwalone + projektowane naraz.
    // Strefy widoczne od skali 1:5000, wiec ciasny widok dzialki (margines 0.3).
    const mbPog = margines(bbox, 0.3), sPog = mbPog.join(','), pogWH = wymiary(mbPog);
    // Uchwalone POG (widoczne pod projektowanymi)
    const pogUchwUrl = 'https://mapy.geoportal.gov.pl/wss/ext/PlanyOgolneGmin?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:4326&FORMAT=image/png&TRANSPARENT=true&LAYERS=strefy-planistyczne,obszar-uzupelnienia-zabudowy,obszar-zabudowy-srodmiejskiej,akt-planowania-przestrzennego&STYLES=,,,&WIDTH=' + pogWH.W + '&HEIGHT=' + pogWH.H + '&BBOX=' + sPog;
    kolejka.push(function () {
      setMapaOverlay('map-pog', ortoBase(sPog, pogWH), pogUchwUrl);
  ...
  function pobierzLegendePOG(bbox, wh) {
    ...
    const url = 'https://mapy.geoportal.gov.pl/wss/ext/PlanyOgolneGmin'
      ...
      + '&LAYERS=strefy-planistyczne&QUERY_LAYERS=strefy-planistyczne'
  ...
  function rysujObrys(idMapy, wkt, bboxMapy, wh, pokazWymiary) {
    ...
    pierscienie.forEach(function (p) {
      ...
      if (pokazWymiary && punkty.length >= 3) {
        ...
        const odsun = 20;  // piksele — odsuniecie linii tuz poza prostokat

        // LINIA DLUGOSCI: wzdluz krawedzi minP (jeden bok prostokata), odsunieta na zewnatrz.
        // Kierunek "na zewnatrz" = od srodka prostokata w strone tej krawedzi = -pX (bo minP < srodka).
        ...
        // LINIA SZEROKOSCI: wzdluz krawedzi minU, odsunieta na zewnatrz (kierunek -uX).
