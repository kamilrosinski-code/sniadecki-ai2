(function () {
  ...
  function naniesTransakcjeNaMape(txs) {
    ...
    // DOPASUJ obszar mapy do transakcji: jesli ktoras wypada poza domyslny kadr
    // (bo backend rozszerzyl promien szukania), poszerz bbox tak, by objac wszystkie.
    const p = window._wycenaParam;
    const punktyGeo = txs.filter(function (t) { return t.lon && t.lat && t.cenaM2; });
    if (punktyGeo.length) {
      let minLo = bbox[0], minLa = bbox[1], maxLo = bbox[2], maxLa = bbox[3];
      // obejmij dzialke + wszystkie transakcje
      if (p && p.lon) { minLo = Math.min(minLo, p.lon); maxLo = Math.max(maxLo, p.lon); minLa = Math.min(minLa, p.lat); maxLa = Math.max(maxLa, p.lat); }
      punktyGeo.forEach(function (t) {
        minLo = Math.min(minLo, t.lon); maxLo = Math.max(maxLo, t.lon);
        minLa = Math.min(minLa, t.lat); maxLa = Math.max(maxLa, t.lat);
      });
      // Sprawdz, czy transakcje wychodza poza domyslny bbox
      const pozaKadrem = minLo < bbox[0] || maxLo > bbox[2] || minLa < bbox[1] || maxLa > bbox[3];
      if (pozaKadrem) {
        // Poszerz bbox o 12% marginesu i przeladuj podklad na wiekszy obszar
        const mLo = (maxLo - minLo) * 0.12, mLa = (maxLa - minLa) * 0.12;
        bbox = [minLo - mLo, minLa - mLa, maxLo + mLo, maxLa + mLa];
        window._cenyBbox = bbox;
        // Przelicz proporcje pikseli i przeladuj ortofoto na nowy obszar
  ...
  function rysujMapy(bbox, wkt) {
    ...
    kolejka.push(function () { setMapa('map-ceny', cenyOrtoUrl); });
  ...
  // onload/onerror sterują sekwencją. Gdy wszystkie zawioda — przygasza obraz.
  function ladujZFallbackiem(img, zrodla, opts) {
    ...
    img.onerror = function () {
      proba++;
      if (proba < MAX_PROB) {
        setTimeout(sprobuj, 1500);   // ponow to samo zrodlo
      } else {
        zi++; proba = 0;             // przejdz do nastepnego zrodla
        setTimeout(sprobuj, 400);
      }
    };
    img.onload = function () {
      img.style.opacity = '1';
  ...
  function setMapa(id, url) {
    ...
    if (bboxM && wM && hM) {
      ...
      ladujZFallbackiem(img, ortoZrodla(bboxM[1], wM[1], hM[1]));
    } else {
      // Nie-ortofoto (np. warstwa) — proste ponawianie jednego url
      let proba = 0;
      img.onerror = function () {
        if (proba < 2) { proba++; const self = this; setTimeout(function () { self.src = url + '&_r=' + Date.now(); }, 1800); }
        else { this.style.opacity = '0.3'; this.alt = 'Podklad chwilowo niedostepny'; }
      };
  ...
  function setMapaOverlay(id, bazaUrl, warstwaUrl) {
    ...
    // Ortofoto jako tlo — z fallbackiem na kilka zrodel GUGiK.
    // Uzywamy ukrytego obrazu do testowania, ktore zrodlo dziala, potem ustawiamy je jako tlo.
    ...
    // Warstwa na wierzchu; sprobuj do 2 razy (zewnetrzne serwery WMS bywaja wolne)
