(function () {
  ...
  function pobierzCenyZBazy(lat, lon, powierzchnia) {
    ...
    if (BACKEND_CENY_URL && BACKEND_CENY_URL !== 'WKLEJ_TUTAJ_LINK_BACKENDU') {
      const url = BACKEND_CENY_URL + '?lon=' + lon + '&lat=' + lat + '&promien=1500';
      fetch(url)
        .then(function (r) {
  ...
  function rysujObrys(idMapy, wkt, bboxMapy, wh, pokazWymiary) {
    ...
    pierscienie.forEach(function (p) {
      ...
      // WYMIARY jako LINIE WYMIAROWE rownolegle do bokow dzialki (styl OnGeo).
      if (pokazWymiary && punkty.length >= 3) {
        const lonSr = (minLon + maxLon) / 2, latSr = (minLat + maxLat) / 2;
        const mp = punkty.map(function (pt) {
          return {
            mx: (pt.lon - lonSr) * M_NA_STOPIEN_LON,   // metry X
            my: (pt.lat - latSr) * M_NA_STOPIEN_LAT,   // metry Y
            x: pt.x, y: pt.y                            // piksele na mapie
          };
        });

        // Srodek dzialki (do odsuniecia linii na wlasciwa strone)
        const cxM = mp.reduce(function (s, m) { return s + m.mx; }, 0) / mp.length;
        const cyM = mp.reduce(function (s, m) { return s + m.my; }, 0) / mp.length;

        // Znajdz najdluzszy BOK (kolejne pary wierzcholkow, nie przekatna)
        let maxBok = 0, bi = 0;
        for (let i = 0; i < mp.length - 1; i++) {
          const d = Math.hypot(mp[i + 1].mx - mp[i].mx, mp[i + 1].my - mp[i].my);
          if (d > maxBok) { maxBok = d; bi = i; }
        }
        const a = mp[bi], b = mp[bi + 1];   // najdluzszy bok: a -> b

        // Kierunek najdluzszego boku (w metrach i pikselach)
        const dirMx = b.mx - a.mx, dirMy = b.my - a.my;
        const dirLen = Math.hypot(dirMx, dirMy) || 1;
        const uMx = dirMx / dirLen, uMy = dirMy / dirLen;      // wzdluz (metry)
        const pMx = -uMy, pMy = uMx;                           // prostopadly (metry)

        // Rozpietosc dzialki WZDLUZ tego kierunku i W POPRZEK (rzuty wszystkich wierzcholkow)
        ...
        const dlugosc = maxU - minU;      // dlugosc = rozpietosc wzdluz najdluzszego boku
        const szerokosc = maxP - minP;    // szerokosc = rozpietosc prostopadle
        ...
        // Przelicznik metry -> piksele (ze skali najdluzszego boku)
        const pxPerM = Math.hypot(b.x - a.x, b.y - a.y) / dirLen;
        // Wektory w pikselach
        const uPx = (b.x - a.x) / (dirLen * pxPerM || 1), uPy = (b.y - a.y) / (dirLen * pxPerM || 1);
        // Znormalizowane piksele wzdluz i w poprzek
        const bpLen = Math.hypot(b.x - a.x, b.y - a.y) || 1;
        const upx = (b.x - a.x) / bpLen, upy = (b.y - a.y) / bpLen;   // wzdluz (piksele)
        const npx = -upy, npy = upx;                                  // prostopadly (piksele)

        // Srodek dzialki w pikselach
        const cxPx = mp.reduce(function (s, m) { return s + m.x; }, 0) / mp.length;
        const cyPx = mp.reduce(function (s, m) { return s + m.y; }, 0) / mp.length;

        const odsun = 24;  // piksele — jak daleko linia od krawedzi

        // --- LINIA DLUGOSCI: rownolegla do najdluzszego boku, odsunieta na zewnatrz ---
        // Znajdz krawedz dzialki po stronie "dolnej" (minP) i tam odsun linie
        // Pozycja bazowa: skrajne punkty wzdluz osi, rzutowane na linie boku
        // Dwa konce linii dlugosci = skrajne rzuty wzdluz kierunku, na wysokosci krawedzi
        const dlPx = dlugosc * pxPerM;
        // Punkt startowy: najdluzszy bok, przesuniety zeby objac cala dlugosc
        // Uzyj srodka i rozpietosci
        const srU = (minU + maxU) / 2;
        // Srodek boku w pikselach jako punkt odniesienia
        const midX = (a.x + b.x) / 2, midY = (a.y + b.y) / 2;
        // Kierunek "na zewnatrz" (od srodka dzialki do boku)
        let outX = midX - cxPx, outY = midY - cyPx;
        const outLen = Math.hypot(outX, outY) || 1;
        outX /= outLen; outY /= outLen;
        const dlx1 = midX - upx * dlPx / 2 + outX * odsun;
        const dly1 = midY - upy * dlPx / 2 + outY * odsun;
        const dlx2 = midX + upx * dlPx / 2 + outX * odsun;
        const dly2 = midY + upy * dlPx / 2 + outY * odsun;
        etykiety += liniaWymiarowa(dlx1, dly1, dlx2, dly2, Math.round(dlugosc) + ' m');

        // --- LINIA SZEROKOSCI: prostopadla do najdluzszego boku, odsunieta w bok ---
        const szPx = szerokosc * pxPerM;
        // Punkt odniesienia: jeden z koncow boku (a), linia idzie w poprzek
        // Kierunek "na zewnatrz" w osi wzdluznej (od srodka w strone konca a)
        let ox2 = a.x - cxPx, oy2 = a.y - cyPx;
        const ol2 = Math.hypot(ox2, oy2) || 1; ox2 /= ol2; oy2 /= ol2;
        const szMidX = a.x + npx * szPx / 2 + ox2 * odsun;
        const szMidY = a.y + npy * szPx / 2 + oy2 * odsun;
        const szx1 = szMidX - npx * szPx / 2, szy1 = szMidY - npy * szPx / 2;
        const szx2 = szMidX + npx * szPx / 2, szy2 = szMidY + npy * szPx / 2;
        etykiety += liniaWymiarowa(szx1, szy1, szx2, szy2, Math.round(szerokosc) + ' m');
      }
    });
    if (!paths) return;
