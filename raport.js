(function () {
  ...
  function rysujMapy(bbox) {
    ...
    // Kluczowe mapy pierwsze (od razu), dodatkowe warstwy pozniej.
    const kolejka = [];

    // MAPA 0: Zaznaczenie dzialki — ciasny widok
    ...
    kolejka.push(function () { setMapaOverlay('map-zaznaczenie', ortoBase(sZazn, zazn), dzialkiUrl); });

    // MAPA 1: Ortofotomapa
    kolejka.push(function () { setMapa('map-orto', ortoBase(s, orto)); });
    ...
    kolejka.push(function () { setMapaOverlay('map-mpzp', ortoBase(s, orto), mpzpUrl); pobierzLegendeMPZP(bbox, orto); });
    ...
    // MAPA 6: Linie energetyczne (BDOT10k)
    ...
    kolejka.push(function () { setMapaOverlay('map-energia', ortoBase(sCeny, ortoC), energiaUrl); });
