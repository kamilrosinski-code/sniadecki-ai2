(function () {
  ...
  function rysujObrys(idMapy, wkt, bboxMapy, wh, pokazWymiary) {
    ...
    pierscienie.forEach(function (p) {
      ...
      // Wymiary bokow (tylko gdy pokazWymiary) — dla kazdego boku dlugosc w metrach
      if (pokazWymiary) {
        for (let i = 0; i < punkty.length - 1; i++) {
          const a = punkty[i], b = punkty[i + 1];
          const dLon = (b.lon - a.lon) * M_NA_STOPIEN_LON;
          const dLat = (b.lat - a.lat) * M_NA_STOPIEN_LAT;
          const dlugosc = Math.sqrt(dLon * dLon + dLat * dLat);
          if (dlugosc < 1) continue;  // pomin mikroskopijne boki
          const mx = (a.x + b.x) / 2;
          const my = (a.y + b.y) / 2;
          const tekst = dlugosc >= 100 ? Math.round(dlugosc) + ' m' : dlugosc.toFixed(1) + ' m';
          // Tlo etykiety + tekst
          const szer = tekst.length * 15 + 16;
          etykiety += '<g>' +
            '<rect x="' + (mx - szer / 2).toFixed(1) + '" y="' + (my - 16).toFixed(1) + '" width="' + szer + '" height="26" rx="4" fill="rgba(11,12,10,0.82)" stroke="#c9a961" stroke-width="1"/>' +
            '<text x="' + mx.toFixed(1) + '" y="' + (my + 3).toFixed(1) + '" font-family="monospace" font-size="20" fill="#dfc090" text-anchor="middle">' + tekst + '</text>' +
            '</g>';
        }
      }
    });
