(function () {
  ...
  function pokazPoczekalnie() {
    ...
    // Domknij po max 14s (gdyby cos zawieszlo, raport i tak sie pokaze)
    setTimeout(ukryjPoczekalnie, 14000);
  }
  ...
  function rysujMapy(bbox, wkt) {
    ...
    kolejka.push(function () {
      ...
      rysujObrys('map-zaznaczenie', wkt, mbZazn, zazn);
    });
    ...
    kolejka.push(function () { setMapaOverlay('map-energia', ortoBase(sCeny, ortoC), uzbrojenieUrl); rysujObrys('map-energia', wkt, mbCeny, ortoC); });
    ...
    // Ukryj poczekalnie gdy pierwsze mapy zaczely sie ladowac
    // (kolejka + margines na render pierwszych warstw)
    setTimeout(ukryjPoczekalnie, kolejka.length * 350 + 2500);
  }
  ...
  function rysujObrys(idMapy, wkt, bboxMapy, wh) {
    const img = $(idMapy);
    if (!img || !wkt) return;
    const box = img.parentElement;
    if (!box) return;
    ...
    // Wymiary obrazu (piksele) — viewBox SVG bedzie taki sam, wiec proporcje sie zgadzaja
    ...
    pierscienie.forEach(function (p) {
      ...
      wsp.forEach(function (para, i) {
        ...
        // Pozycja w PIKSELACH obrazu (0..W, 0..H) — obraz wypelnia bbox liniowo
        const x = ((lon - minLon) / szerLon) * W;
        const y = ((maxLat - lat) / szerLat) * H;
        d += (i === 0 ? 'M' : 'L') + x.toFixed(1) + ',' + y.toFixed(1) + ' ';
      });
      if (d) paths += '<path d="' + d + 'Z" fill="rgba(201,169,110,0.18)" stroke="#c9a961" stroke-width="2.5" vector-effect="non-scaling-stroke" stroke-linejoin="round"/>';
    });
    if (!paths) return;
    ...
    // WAZNE: takie samo dopasowanie jak object-fit:cover na obrazie
