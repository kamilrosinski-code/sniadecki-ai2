(function () {
      ...
      fetch(url)
        ...
        .then(function (dane) {
          ...
          if (dane.mediana_cena_m2 && powierzchnia && $('pods-cena')) {
            ...
            if ($('pods-cena-sub')) {
              // Pokaz faktyczny promien (backend rozszerza go na terenach z mala liczba transakcji)
              ...
              if (dane.rozszerzony) opis += ' (poszerzony — malo transakcji w okolicy)';
              $('pods-cena-sub').textContent = opis;
            }
          }
  ...
  function rysujTransakcjeZBazy(txs) {
    ...
    if (txs.length > WIDOCZNE) {
      ...
      btn.addEventListener('click', function () {
        ...
        table.querySelectorAll('.tx-ukryta').forEach(function (el) {
          el.style.display = rozwiniete ? '' : 'none';
        });
  ...
  function rysujMapy(bbox, wkt) {
    ...
    const ortoBaseCeny = function (bb, wh) {
      let W = wh.W, H = wh.H;
      const max = 1024;
      if (W > max || H > max) {
  ...
  function setMapa(id, url) {
    ...
    img.onerror = function () {
      // Ponow do 2 razy (Geoportal czesto odpowiada za drugim/trzecim razem)
