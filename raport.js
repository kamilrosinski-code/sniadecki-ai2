(function () {
  ...
  function rysujMapy(bbox) {
    ...
    // Bazowa ortofotomapa (wspolny podklad)
    const ortoBase = 'https://mapy.geoportal.gov.pl/wss/service/PZGIK/ORTO/WMS/StandardResolution?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:4326&FORMAT=image/png&TRANSPARENT=false&LAYERS=Raster&STYLES=&WIDTH=' + W + '&HEIGHT=' + H;

    // MAPA 1: Ortofotomapa + dzialki ewidencyjne
    setMapa('map-orto', ortoBase + '&BBOX=' + s);

    // MAPA 2: MPZP nalozone na ortofoto (dwie warstwy w jednym elemencie)
    // Warstwy MPZP: plany + raster (wiekszosc gmin ma plany rastrowe) + granice
    const mpzpUrl = 'https://mapy.geoportal.gov.pl/wss/ext/KrajowaIntegracjaMiejscowychPlanowZagospodarowaniaPrzestrzennego?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:4326&FORMAT=image/png&TRANSPARENT=true&LAYERS=plany,raster,granice&STYLES=&WIDTH=' + W + '&HEIGHT=' + H + '&BBOX=' + s;
    setMapaOverlay('map-mpzp', ortoBase + '&BBOX=' + s, mpzpUrl);

    // MAPA 3: Ceny transakcyjne (RCN) nalozone na ortofoto
    // Poprawny adres: /wss/service/rcn — warstwy: dzialki, budynki, lokale
    const cenyUrl = 'https://mapy.geoportal.gov.pl/wss/service/rcn?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:4326&FORMAT=image/png&TRANSPARENT=true&LAYERS=dzialki,budynki,lokale&STYLES=&WIDTH=' + W + '&HEIGHT=' + H + '&BBOX=' + mbCeny;
    setMapaOverlay('map-ceny', ortoBase + '&BBOX=' + mbCeny, cenyUrl);
  }
  ...
  function setMapaOverlay(id, bazaUrl, warstwaUrl) {
    ...
    // Ustaw ortofoto jako tlo elementu, a warstwe jako <img> na wierzchu
