(function () {
  ...
  function rysujMapy(bbox, wkt) {
    ...
    const mbZazn = margines(bbox, 0.4), sZazn = mbZazn.join(','), zazn = wymiary(mbZazn);
    ...
    // KIUT (uzbrojenie) wyswietla sie dopiero przy DUZYM przyblizeniu (~1:500),
    // dlatego ciasny widok dzialki (margines 0.2), nie szeroki — inaczej warstwa jest pusta.
    const mbUzbr = margines(bbox, 0.2), sUzbr = mbUzbr.join(','), uzbrWH = wymiary(mbUzbr);
    // Nazwy warstw wg ZYWEJ uslugi KIUT: przewod_* (nie siec_*)
    const uzbrojenieUrl = 'https://integracja.gugik.gov.pl/cgi-bin/KrajowaIntegracjaUzbrojeniaTerenu?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:4326&FORMAT=image/png&TRANSPARENT=true&LAYERS=przewod_wodociagowy,przewod_kanalizacyjny,przewod_cieplowniczy,przewod_gazowy,przewod_telekomunikacyjny,przewod_elektroenergetyczny,przewod_niezidentyfikowany,przewod_specjalny,urzadzenia_towarzyszace&STYLES=,,,,,,,,&WIDTH=' + uzbrWH.W + '&HEIGHT=' + uzbrWH.H + '&BBOX=' + sUzbr;
  ...
  function rysujObrys(idMapy, wkt, bboxMapy, wh, pokazWymiary) {
    ...
    pierscienie.forEach(function (p) {
      ...
      if (pokazWymiary && punkty.length >= 3) {
        ...
        // Srodek dzialki w metrach — punkt odniesienia dla naroznikow prostokata
        const cU = (minU + maxU) / 2, cP = (minP + maxP) / 2;
        const cxPx = mp.reduce(function (s, m) { return s + m.x; }, 0) / mp.length;
        const cyPx = mp.reduce(function (s, m) { return s + m.y; }, 0) / mp.length;

        // Funkcja: pozycja (u_metry, p_metry) wzgledem srodka -> piksele
        const naPiksele = function (uM, pM) {
          const du = (uM - cU) * pxPerM, dp = (pM - cP) * pxPerM;
          return { x: cxPx + uX * du + pX * dp, y: cyPx + uY * du + pY * dp };
        };

        const dlPx = dlugosc * pxPerM, szPx = szerokosc * pxPerM;
        const odsun = 22;  // piksele — odsuniecie linii poza prostokat

        // LINIA DLUGOSCI: gorna krawedz prostokata (strona minP), odsunieta na zewnatrz
        const dA = naPiksele(minU, minP);
        const dB = naPiksele(maxU, minP);
        etykiety += liniaWymiarowa(
          dA.x - pX * odsun, dA.y - pY * odsun,
          dB.x - pX * odsun, dB.y - pY * odsun,
          Math.round(dlugosc) + ' m'
        );

        // LINIA SZEROKOSCI: lewa krawedz prostokata (strona minU), odsunieta na zewnatrz
        const sA = naPiksele(minU, minP);
        const sB = naPiksele(minU, maxP);
        etykiety += liniaWymiarowa(
          sA.x - uX * odsun, sA.y - uY * odsun,
          sB.x - uX * odsun, sB.y - uY * odsun,
          Math.round(szerokosc) + ' m'
        );
      }
    });
    if (!paths) return;
