...
(function () {
  ...
  const SHEET_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSbR8JiZTcGO4fTshrnGgqZhSr3BYEi8hCEcb2EKr2CfcsO6xlwaEkTbH1F9xt015D-jf_0iPnMfTvM/pub?single=true&output=csv';
  ...
  function animujEtapy() {
    ...
    if (!strzalka || !etapy.length || !linia) return;
    let i = 0;
    const przejdz = function () {
      etapy.forEach(function (e, idx) { e.classList.toggle('etap-aktywny', idx === i); });
      const etap = etapy[i];
      const liniaBox = linia.getBoundingClientRect();
      const numEl = etap.querySelector('.etap-num');
      const numBox = numEl.getBoundingClientRect();
      const top = (numBox.top + numBox.height / 2) - liniaBox.top - 16;
      strzalka.style.top = Math.max(0, top) + 'px';
      i = (i + 1) % etapy.length;
    };
    przejdz();
    setInterval(przejdz, 2200);
  }
  (function () {
    const sekcja = document.getElementById('etapy');
    if (!sekcja) return;
    if ('IntersectionObserver' in window) {
      let uruchomiono = false;
      const obs = new IntersectionObserver(function (wpisy) {
        wpisy.forEach(function (w) {
          if (w.isIntersecting && !uruchomiono) { uruchomiono = true; setTimeout(animujEtapy, 300); }
        });
      }, { threshold: 0.3 });
      obs.observe(sekcja);
    } else {
      setTimeout(animujEtapy, 500);
    }
  })();
