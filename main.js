...
(function () {
  ...
  // Animacja naglowka hero — subtelne wylanianie sie LINIAMI (nie slowo po slowie).
  // Styl premium: kazda linia plynnie pojawia sie z lekkim ruchem w gore.
  function animujNaglowek() {
    const h = document.querySelector('.hero-headline');
    if (!h || h.dataset.animowany) return;
    h.dataset.animowany = '1';
    // Rozbij na LINIE wg <br>. Kazda linia (z ewentualnym <em>) idzie w osobny wrapper.
    const html = h.innerHTML;
    const linie = html.split(/<br\s*\/?>/i);
    h.innerHTML = linie.map(function (linia) {
      return '<span class="hw-linia">' + linia + '</span>';
    }).join('<br>');
    // Pokaz linie kolejno — plynnie, z niewielkim odstepem
    const spany = h.querySelectorAll('.hw-linia');
    spany.forEach(function (sp, i) {
      setTimeout(function () { sp.classList.add('hw-widoczne'); }, 250 + i * 450);
    });
  }
