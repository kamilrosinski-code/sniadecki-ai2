...
(function () {
  ...
  fetch(SHEET_CSV_URL)
    ...
    .then(function (csv) {
      ...
      // Po podstawieniu treści z arkusza — animuj naglowek slowo po slowie
      animujNaglowek();
    })
    .catch(function (e) {
      console.warn('CMS: nie udało się pobrać arkusza —', e);
      // Strona pokaże domyślne teksty z HTML
      animujNaglowek();  // animuj domyslny tekst
    });

  // Animacja naglowka hero — kazde slowo pojawia sie po kolei po wejsciu na strone.
  function animujNaglowek() {
    const h = document.querySelector('.hero-headline');
    if (!h || h.dataset.animowany) return;
    h.dataset.animowany = '1';
    // Zachowaj strukture: <br> i <em>. Rozbij tekst na slowa, opakuj kazde w span.
    // Przechodzimy po wezlach: tekst -> slowa w spanach, <br> i <em> zostaja.
    const owinSlowa = function (node) {
      const dzieci = Array.prototype.slice.call(node.childNodes);
      dzieci.forEach(function (n) {
        if (n.nodeType === 3) {  // tekst
          const slowa = n.textContent.split(/(\s+)/);  // zachowaj spacje
          const frag = document.createDocumentFragment();
          slowa.forEach(function (s) {
            if (s.trim() === '') { frag.appendChild(document.createTextNode(s)); return; }
            const span = document.createElement('span');
            span.className = 'hw-slowo';
            span.textContent = s;
            frag.appendChild(span);
          });
          node.replaceChild(frag, n);
        } else if (n.nodeType === 1 && n.tagName === 'EM') {
          owinSlowa(n);  // wejdz do <em>, owin slowa w srodku
        }
        // <br> zostaje bez zmian
      });
    };
    owinSlowa(h);
    // Pokaz slowa kolejno
    const spany = h.querySelectorAll('.hw-slowo');
    spany.forEach(function (sp, i) {
      setTimeout(function () { sp.classList.add('hw-widoczne'); }, 180 + i * 220);
    });
  }
